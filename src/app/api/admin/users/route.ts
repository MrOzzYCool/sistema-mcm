...
export async function GET(req: NextRequest) {
  ...
  const { data, error } = await supabaseAdmin.from("profiles")
    .select("id, nombre_completo, rol, estado, dni, genero, es_profesor, created_at")
...
export async function PUT(req: NextRequest) {
  ...
  const { userId, nombre_completo, email, rol, estado, dni, genero, es_profesor, ciclo } = await req.json();
