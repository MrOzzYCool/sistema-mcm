...
const INIT: FormState = {
  ...
  celular: "", actualizacionOrigen: "", certificadoId: "",
...
export default function CertificadosActualizacionPage() {
  ...
  async function handleSubmit(e: React.FormEvent) {
    e.preventDefault();
    if (!puedeEnviar || !certificado) return;
    setError("");
    setSubmitting(true);
    ...
    <div className="min-h-screen" style={{ background: "linear-gradient(160deg,#a93526 0%,#8a2b1f 100%)" }}>
      ...
      <header className="flex items-center gap-4 px-6 py-4 border-b border-white/10">
        ...
        <div>
          ...
          <p className="text-white/60 text-xs">Solicitud de Certificados — Actualizaciones</p>
        </div>
      </header>
          ...
          <form onSubmit={handleSubmit} className="space-y-5">
            ...
            <fieldset>
              ...
              <div className="grid sm:grid-cols-2 gap-4">
                ...
                <EmailConfirmField value={form.emailConfirm} onChange={(v) => set("emailConfirm", v)} emailOriginal={form.email} />
              </div>
            </fieldset>
