...
/** Get the post-password-change destination based on role */
...
export default function CambiarContrasenaPage() {
  ...
  const valid = password.length >= MIN_PASSWORD_LENGTH && match;

  // If user is not authenticated or doesn't need password reset, redirect away
  useEffect(() => {
    ...
    if (!user) {
      ...
      router.replace(getDestinationByRole(user.role));
    }
  ...
  async function handleSubmit(e: React.FormEvent) {
    ...
    try {
      ...
      // Determine redirect destination from API response or current user role
      const role = json.role ?? user?.role ?? "alumno";
      const destination = getDestinationByRole(role);

      // Force a full page reload to re-resolve the user (flag is now cleared)
      // This ensures the auth context picks up forcePasswordReset=false
  ...
  // If not authenticated, render nothing (redirect is happening)
  if (!user) return null;
        ...
        <div className="bg-white rounded-2xl shadow-2xl p-8">
          <div className="flex items-center gap-3 mb-6">
            ...
            <div>
              ...
              <p className="text-mcm-muted text-xs">
          ...
          <form onSubmit={handleSubmit} className="space-y-4">
            ...
            <div>
              ...
              {confirm && match && password.length >= MIN_PASSWORD_LENGTH && (
