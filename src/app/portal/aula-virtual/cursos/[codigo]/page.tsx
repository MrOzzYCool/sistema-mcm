...
function getStatusBadge(estado: string) {
  switch (estado) {
    case "pendiente":
      return <span className="inline-flex items-center gap-1 text-xs px-2.5 py-1 rounded-full bg-yellow-100 text-yellow-800"><Clock size={12} /> Pendiente</span>;
    case "entregado":
      return <span className="inline-flex items-center gap-1 text-xs px-2.5 py-1 rounded-full bg-green-100 text-green-800"><CheckCircle2 size={12} /> Entregado</span>;
    case "vencido":
      return <span className="inline-flex items-center gap-1 text-xs px-2.5 py-1 rounded-full bg-red-100 text-red-800"><AlertCircle size={12} /> Vencido</span>;
    default: return null;
  }
}
...
export default function CourseDetailPage() {
    ...
    async function fetchData() {
      try {
        ...
        const { data: contentData } = await supabase
          .from("contenido_semanas").select("*").eq("curso_id", courseData.id).order("semana", { ascending: true });
        setContent(contentData && contentData.length > 0 ? contentData : seedContent);

        const { data: tareasData } = await supabase
          .from("entregas").select("*").eq("curso_id", courseData.id);
  ...
  const openUploadModal = (tarea: Tarea) => { setSelectedTarea(tarea); setShowUploadModal(true); };

  if (loading) return (
    <div className="flex items-center justify-center min-h-[50vh]">
      <p className="text-mcm-muted text-lg animate-pulse">Cargando curso...</p>
    </div>
  );
  ...
  return (
    <div className="p-4 w-full max-w-[1200px] mx-auto">
      ...
      <div className="relative rounded-xl overflow-hidden mb-6 shadow-sm">
        ...
        <img
          src={(course.imagen_url && course.imagen_url.trim() !== "") ? course.imagen_url : `https://picsum.photos/seed/${codigo}/1200/300`}
          alt={displayName}
          ...
          <div className="flex flex-col sm:flex-row sm:items-end sm:justify-between gap-4">
            <div>
              ...
              <div className="flex items-center gap-2 mt-2 text-sm text-white/90">
                <User size={14} />
                <span>{course.profesor || "Docente no asignado"}</span>
              </div>
              ...
              <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-3">
                <div className="flex-1">
                  ...
                  <p className="text-xs text-gray-500 mt-1">
                    Fecha de entrega: {new Date(tarea.fecha_entrega).toLocaleDateString("es-PE", { day: "2-digit", month: "long", year: "numeric" })}
                  </p>
                </div>
                <div className="flex items-center gap-3">
